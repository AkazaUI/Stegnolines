...
/**
 * Domain-specific cryptographic salt for PBKDF2 key derivation.
 * Ensures derived keys are tied exclusively to StegoLine's PRNG domain.
 */
...
]); // "StegoLine::CSPRNG::v2::Salt2026!"

/** Number of PBKDF2 HMAC-SHA256 iterations (Standard NIST/OWASP Recommendation) */
const PBKDF2_ITERATION_COUNT = 100000;

/** Cache storing previously derived 256-bit keys for instant O(1) lookups */
const _derivedKeyCache = new Map();

// ──────────────────────────────────────────────────────────────
// MODULE 1: SHA-256 & HMAC-SHA256 Core Engine (Bitwise Optimized)
// ──────────────────────────────────────────────────────────────
...
/**
 * Computes SHA-256 hash of an arbitrary byte buffer.
 *
 * @param {Uint8Array} messageBytes - Data bytes to hash.
...
/**
 * Computes HMAC-SHA256 of messageBytes using keyBytes.
 *
 * @param {Uint8Array} keyBytes     - Authentication key.
 * @param {Uint8Array} messageBytes - Data bytes to authenticate.
 ...
 *
 * @param {string}     stegoKey   - The user-supplied secret passphrase.
 * @param {Uint8Array} saltBytes  - Domain salt bytes.
 * @param {number}     iterations - Number of derivation rounds (100,000).
 * @returns {Uint8Array} 32-byte (256-bit) cryptographic key.
 */
function deriveKeyPbkdf2(stegoKey, saltBytes = STEGO_DOMAIN_SALT_BYTES, iterations = PBKDF2_ITERATION_COUNT) {
  ...
  const encoder = typeof TextEncoder !== 'undefined' ? new TextEncoder() : { encode: (s) => Uint8Array.from(unescape(encodeURIComponent(s)), c => c.charCodeAt(0)) };
  const passwordBytes = encoder.encode(stegoKey);

  // Pre-compute key padding for HMAC optimization
  ...
  // Pre-allocated buffers for zero-allocation stretching loop
  ...
  // PBKDF2 Block 1: Salt || INT_32_BE(1)
  ...
  // Key Stretching Loop (100,000 rounds)
...
// ──────────────────────────────────────────────────────────────
// MODULE 2: AES-256 Core Cipher & CTR Mode Stream Generator
// ──────────────────────────────────────────────────────────────
...
/**
 * Expands a 256-bit (32-byte) key into AES round keys (60 32-bit words).
 *
 * @param {Uint8Array} keyBytes - 32-byte AES-256 key.
 * @returns {Uint32Array} 60 expanded round key words.
 */
...
/**
 * Encrypts a 16-byte block using AES-256 (FIPS-197 Standard).
 *
 * @param {Uint32Array} expandedKey - 60 round key words.
 * @param {Uint8Array}  inBlock     - 16-byte plaintext/counter block.
 * @param {Uint8Array}  outBlock    - 16-byte ciphertext block buffer.
 */
...
/**
 * Creates an AES-256-CTR Cryptographically Secure PRNG (CSPRNG).
 *
 * @param {Uint8Array} key256Bytes - 32-byte AES key derived via PBKDF2.
 * @returns {{ nextUint32: function(): number }} CSPRNG generator object.
 */
...
// ──────────────────────────────────────────────────────────────
// MODULE 3: Unbiased Rejection Sampling Engine (Zero-Freeze)
// ──────────────────────────────────────────────────────────────

/**
 * Draws an unbiased random integer in the range [0, range - 1].
 *
 * Rejection Sampling Algorithm:
 * Calculates the largest multiple of `range` within [0, 2^32 - 1] (Valid Threshold).
 * Any raw uint32 falling in the residual zone is rejected and redrawn,
 * eliminating Modulo Bias completely to pass Steganalysis Chi-Square tests.
 *
 * Defensive Bugfix:
 * - When remainder === 0 (e.g. range is a power of 2 or divides 2^32), validThreshold
 *   evaluates to 4294967296 (no rejection needed), preventing 32-bit integer wrap to 0.
 *
 * @param {number} range   - Upper exclusive bound (range > 0).
 * @param {object} csprng  - AES-256-CTR CSPRNG instance.
 * @returns {number} Uniform integer in [0, range - 1].
 */
function getUnbiasedRandomInt(range, csprng) {
  if (range <= 1) return 0;

  const LIMIT_32 = 4294967296; // 2^32
  if (range >= LIMIT_32) return csprng.nextUint32();

  const remainder = LIMIT_32 % range;
  // If remainder === 0, validThreshold is exactly 4294967296 (all 32-bit uints are valid)
  ...
  do {
    ...
    if (++safetyCounter > 1000) break; // Defensive guard against impossible stalls
...
// ──────────────────────────────────────────────────────────────
// MODULE 4: O(K) Lazy Swap Shuffle Engine (Stego Positions)
// ──────────────────────────────────────────────────────────────
 ...
 *
 * Modern Architecture:
 * 1. Derives 256-bit key via PBKDF2-HMAC-SHA256 (100,000 rounds).
 * 2. Seeds AES-256-CTR CSPRNG for cryptographic stream generation.
 * 3. Applies Unbiased Rejection Sampling to eliminate statistical Modulo Bias.
 * 4. Executes Fisher-Yates shuffle lazily using a Map:
 *    - Memory Complexity: O(K) — zero full-array RAM allocation.
 *    - Time Complexity:   O(K) — executes in microseconds regardless of carrier size.
 *
 * @param {number} maxLength - Total available carrier positions (cover bit length).
 * @param {number} count     - How many unique secret positions to select (K bits).
 * @param {string} stegoKey  - The secret stego-key passphrase.
 * @returns {number[]} Array of `count` unique position indices.
 */
...
/**
 * Resolve the stego-key for encryption and position seeding.
 *
