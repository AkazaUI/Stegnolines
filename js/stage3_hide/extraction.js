...
function displayExtractionResults(secretMessage, hint) {
  ...
  if (hint) {
    ...
    document.getElementById('extractHintText').textContent = 'The next message will be here ☝️';
...
async function performExtraction() {
  const stegoText = document.getElementById('extractCover').value.trim();
  const rawStegoKey = document.getElementById('extractStegoKey').value.trim();

    if (!stegoText.trim()) return showToast('⚠ Please input the final message.');
    if (!rawStegoKey.trim()) return showToast('⚠ Please input the Pre-Shared Key (Stego-Key).');
    ...
    // ── Stage 2 (Reverse): AES-256-CTR Decryption (0-byte overhead)
    ...
    const decryptionKey = (encryptionKeyEl && encryptionKeyEl.value.trim()) || resolvedStegoKey;
