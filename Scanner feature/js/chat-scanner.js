...
(function () {
  ...
  function initChatScanner(container, showToast, t) {
          ...
          <div id="results-dashboard" style="display: none;">
            ...
            <div class="date-range-filter-card">
              <div class="date-range-filter__header">
                <div class="date-range-filter__title-wrap">
                  <span class="material-symbols-outlined date-range-filter__icon">date_range</span>
                  <span class="date-range-filter__title">${t('dateRangeFilter')}</span>
                </div>
                <span class="badge badge--draft date-range-filter__status" id="date-filter-status">${t('allDates')}</span>
              </div>
              <div class="date-range-filter__grid">
                <div class="date-range-filter__field">
                  <label class="date-range-filter__label" for="input-date-from">${t('fromDate')}</label>
                  <div class="date-range-filter__input-wrap">
                    <span class="material-symbols-outlined date-range-filter__input-icon">schedule</span>
                    <input type="text" id="input-date-from" class="date-range-filter__input" placeholder="${t('datePlaceholder')}" autocomplete="off" />
                  </div>
                </div>
                <div class="date-range-filter__field">
                  <label class="date-range-filter__label" for="input-date-to">${t('toDate')}</label>
                  <div class="date-range-filter__input-wrap">
                    <span class="material-symbols-outlined date-range-filter__input-icon">schedule</span>
                    <input type="text" id="input-date-to" class="date-range-filter__input" placeholder="${t('datePlaceholder')}" autocomplete="off" />
                  </div>
                </div>
              </div>
              <div class="date-range-filter__presets">
                <button type="button" class="date-preset-btn" data-preset="today">${t('presetToday')}</button>
                <button type="button" class="date-preset-btn" data-preset="7days">${t('presetLast7')}</button>
                <button type="button" class="date-preset-btn" data-preset="30days">${t('presetLast30')}</button>
                <button type="button" class="date-preset-btn" data-preset="year">${t('presetThisYear')}</button>
                <button type="button" class="date-preset-btn date-preset-btn--reset" data-preset="reset">${t('presetReset')}</button>
              </div>
            </div>
    ...
    function attachEventListeners() {
          ...
          const fpConfig = {
            ...
            onChange: function(selectedDates, dateStr, instance) {
              if (instance.element.id === 'input-date-from') {
                ...
                if (toEl && toEl._flatpickr && selectedDates.length > 0) {
                  toEl._flatpickr.set('minDate', selectedDates[0]);
                }
              } else {
                dateTo = dateStr;
                const fromEl = container.querySelector('#input-date-from');
                if (fromEl && fromEl._flatpickr && selectedDates.length > 0) {
      ...
      container.querySelectorAll('.date-preset-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          const msgs = (lastParseResult && lastParseResult.messages) || [];
          const validDates = msgs.map(m => m.timestamp ? parseTimestamp(m.timestamp) : null).filter(Boolean);
          ...
          if (preset === 'reset' || !minDate || !maxDate) {
            dateFrom = '';
            dateTo = '';
          } else if (preset === 'today') {
            dateFrom = fmtD(maxDate) + ' 00:00';
            dateTo = fmtDT(maxDate);
          } else if (preset === '7days') {
            const d = new Date(maxDate); d.setDate(d.getDate() - 7);
            dateFrom = fmtDT(d < minDate ? minDate : d);
            dateTo = fmtDT(maxDate);
          } else if (preset === '30days') {
            const d = new Date(maxDate); d.setDate(d.getDate() - 30);
            dateFrom = fmtDT(d < minDate ? minDate : d);
            dateTo = fmtDT(maxDate);
          } else if (preset === 'year') {
            const yearStart = new Date(maxDate.getFullYear(), 0, 1, 0, 0, 0);
            dateFrom = fmtDT(yearStart < minDate ? minDate : yearStart);
            dateTo = fmtDT(maxDate);
          }
          ...
          if (fpFrom) {
            if (minDate) fpFrom.set('minDate', minDate);
            if (maxDate) fpFrom.set('maxDate', dateTo ? new Date(dateTo.replace(' ', 'T')) : maxDate);
            fpFrom.setDate(dateFrom, false);
          }
          if (fpTo) {
            if (minDate) fpTo.set('minDate', dateFrom ? new Date(dateFrom.replace(' ', 'T')) : minDate);
            if (maxDate) fpTo.set('maxDate', maxDate);
            fpTo.setDate(dateTo, false);
          }

          container.querySelectorAll('.date-preset-btn').forEach(b => {
            if (preset !== 'reset' && b.dataset.preset === preset) b.classList.add('active');
    ...
    function renderDashboard(result) {
      ...
      // Configure Date Range Filter bounds if messages have dates
      const dateCard = container.querySelector('.date-range-filter-card');
      const validDates = msgs.map(m => m.timestamp ? parseTimestamp(m.timestamp) : null).filter(Boolean);
      if (validDates.length > 0) {
        const minTime = Math.min(...validDates.map(d => d.getTime()));
        const maxTime = Math.max(...validDates.map(d => d.getTime()));
        ...
        if (fromEl && fromEl._flatpickr) {
          fromEl._flatpickr.set('minDate', minD);
          fromEl._flatpickr.set('maxDate', maxD);
        }
        if (toEl && toEl._flatpickr) {
          toEl._flatpickr.set('minDate', minD);
          toEl._flatpickr.set('maxDate', maxD);
        }
      } else {
    ...
    function renderMessagesList() {
      ...
      if (dateFrom || dateTo) {
        const parseDT = (str, isEnd) => {
          if (!str) return null;
          str = str.trim();
          if (str.length <= 10) return isEnd ? new Date(str + 'T23:59:59.999') : new Date(str + 'T00:00:00');
          const norm = str.replace(' ', 'T');
          const d = new Date(norm + (norm.length <= 16 ? ':00' : ''));
          return isNaN(d.getTime()) ? new Date(str) : d;
        };
        const from = parseDT(dateFrom, false);
        const to = parseDT(dateTo, true);
        msgs = msgs.filter(m => {
  ...
  window.ChatScanner = {
    initChatScanner,
    applyLanguage: function (lang) {
      if (globalInstance && typeof globalInstance.applyLanguage === 'function') {
        globalInstance.applyLanguage(lang);
      }
    }
  };
})();
