// ══════════════════════════════════════════════════════════════
// Stage 3 — Hide | Step 1: Stego-Payload Codec
// ══════════════════════════════════════════════════════════════
//
...
function buildPayload(secretMessage, hint) {
  ...
  const secretMessageBytes = encoder.encode(secretMessage || '');
  ...
  // ── With hint → delimiter format (1 byte overhead)
  const hintBytes = encoder.encode(trimmedHint);
...
function parsePayload(bytes) {
  const decoder = typeof SHARED_TEXT_DECODER !== 'undefined' ? SHARED_TEXT_DECODER : new TextDecoder();

  // Scan for the delimiter byte
  const delimiterIndex = bytes.indexOf(HINT_DELIMITER);

  if (delimiterIndex !== -1) {
    const secretMessage = decoder.decode(bytes.subarray(0, delimiterIndex));
    const hint = decoder.decode(bytes.subarray(delimiterIndex + 1));
    return { secretMessage, hint };
  }

  // No delimiter — entire payload is the raw secret message
  return { secretMessage: decoder.decode(bytes), hint: '' };
}
