...
function displayExtractionResults(secretMessage, hint) {
  ...
  if (hint) {
    ...
    document.getElementById('extractHintText').textContent = 'textmessage text text text ☝️';
...
async function performExtraction() {
  ...
  if (!stegoObject.trim()) return showToast('⚠ text text textmessage text.');
  ...
  try {
    ...
    if (vsBytes.length === 0) {
      throw new Error('text text text hidden (VS) text textmessage. text text text textmessage text text.');
    }
    ...
    if (xorKeyBinary.length > coverBits.length) {
      throw new Error(`text text ${xorKeyBinary.length} bits text Cover text text ${coverBits.length} bits text.`);
    }
    ...
    showToast('✅ Extracted textmessage text!');
