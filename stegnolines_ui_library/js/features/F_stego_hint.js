...
function getHintTypeConfig(type) {
  if (type === 'sent') {
    return { label: '↗ text', icon: '↗', cssClass: 'hint-type-sent', emptyMessage: 'text text Hinttext text text' };
  }
  return { label: '↙ text', icon: '↙', cssClass: 'hint-type-received', emptyMessage: 'text text Hinttext text text' };
}
...
async function clearHintsLog() {
  let confirmed = false;
  if (typeof showClearHintsConfirmModal === 'function') {
    confirmed = await showClearHintsConfirmModal();
  } else {
    const lang = (typeof getCurrentLang === 'function' ? getCurrentLang() : 'en');
    confirmed = confirm(lang === 'ar' ? 'هل تريد مسح سجل التلميحات بالكامل؟' : 'Do you want to clear all saved hints?');
  }
  if (!confirmed) return;
    ...
    document.getElementById('latestHintContent').innerHTML = `
      <div class="hint-active-inner">
        ...
        <div class="hint-active-info">
          ...
          <span class="hint-time">${escapeHtml(formattedTime)}</span>
        </div>
      </div>
    `;
  } else if (latestCard) {
